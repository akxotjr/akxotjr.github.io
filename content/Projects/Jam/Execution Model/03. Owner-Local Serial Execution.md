...
각 shard의 `ShardLocal`에는 ECS registry와 세션, 사용자, 월드 상태가 저장됩니다. [[01. State Ownership & Task Routing|State Ownership & Task Routing]]에서 선택한 route에 따라 같은 owner의 job이 같은 shard로 전달되며, shard main thread가 이를 직렬로 실행합니다.
...
shard ingress는 shard 전체를 향한 작업을 받고, ready mailbox는 특정 owner를 향한 작업을 받습니다. 두 경우 모두 실제 callback은 shard main loop가 실행합니다. producer 측 전달, stale reference 검증, mailbox lifecycle은 [[04. Cross-Thread Task Dispatch|Cross-Thread Task Dispatch]]에서 이어서 설명합니다.
...
fiber의 내부 scheduling과 global scheduler와의 차이는 [[05. Fiber-Based Async Scheduling|Fiber-Based Async Scheduling]]에서 다룹니다.
...
이 배치와 fallback 정책은 [[06. Thread and Core Topology|Thread and Core Topology]]에서 설명합니다.
